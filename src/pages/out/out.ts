...
export class OutPage extends BaseUI {
  ...
  CurrPlant: string;                       //当前工厂
  CurrWorkShop: string;                    //当前车间
  ...
  ScanSheet() {
    ...
    this.api.get('WM/GetAboutIssueRequest', {requestNo: this.ScanNo, userId: 713}).subscribe((res: any) => {
        if (res.successful) {
          ...
          this.ScanFlag = 1;
          this.SheetNo = this.ScanNo;
          this.ScanPlaceHolder = "请扫描料箱";
          this.ScanNo = "";                                 //扫描框设置为空
          this.CurrPlant = res.data.Sheet.plant;            //工厂
          this.CurrWorkShop = res.data.Sheet.supplier_id;   //车间
        } else {
    ...
    this.api.get('WM/GetUnStandModifyNumber', {
      ...
      IsOutStock:true
        ...
        if (res.successful) {
          if(res.data.length>0){
            for(let partInfo of res.data) {
              let index = this.SheetDetail.indexOf(item => item.id ===partInfo.id)
              if(index>0){
                this.SheetDetail[index].received_pack_count=partInfo.received_pack_count;
                this.SheetDetail[index].received_part_count=partInfo.received_part_count;
                this.RefreshDataModal();
