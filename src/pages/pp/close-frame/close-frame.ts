...
export class CloseFramePage extends BaseUI {
    ...
    scan() {
        ...
        else if (this.sourceItem.parts.findIndex(p => p.boxLabel === this.code) >= 0) {
            err = `料箱${this.code}已扫描过，请扫描其他标签`;
        }

        else if (this.targetItem.parts.findIndex(p => p.boxLabel === this.code) >= 0) {
            err = `料箱${this.code}已扫描过，请扫描其他标签`;
        }
    ...
    scanSheet() {
        this.api.get('PP/GetCloseFrame', { plant: this.api.plant, workshop: this.workshop, box_label: this.code }).subscribe((res: any) => {
            if (res.successful) {
                ...
                else if (this.targetItem.parts.findIndex(p => p.boxLabel === model.boxLabel) >= 0) {
                    this.insertError(`料箱${model.boxLabel}已扫描过，请扫描其他标签`);
                    this.nativeAudio.play('no').then(this.onSuccess, this.onError);
                    return;
                }
